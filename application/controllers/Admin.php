<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Admin extends CI_Controller {
		...
		// 	$siswa = $this->mahasiswa->getMahasiswa();

	 //        $data['mhs']   = $this->mhs->data();
	 //        $data['siswa']    = $siswa->num_rows();

	 //        $this->load->view('admin/lapmahasiswa', $data);
		// }
		...
		public function krs() {
			$data = array(
			'title' => 'Daftar KRS Mahasiswa | Panel Admin SIINI',
			'krs' => $this->m_krs->data()
			);
			$this->load->view('admin/header', $data);
			$this->load->view('admin/krs_list');
			$this->load->view('admin/footer');
		}


		//tambah nilai
		public function krs_tambah() {
			if (isset($_POST['submit'])) {
				$this->m_krs->simpan();
				$this->session->set_flashdata('simpan', 'Nilai Mahasiswa baru berhasil tersimpan ...');
				redirect('admin/krs');
				...
				$data = array(
				'title' => 'Transkasi KRS | Panel Admin SIINI',				
				'mtk' => $this->m_matkul->data()
				);
				$this->load->view('admin/header', $data);
				$this->load->view('admin/krs_tambah');
				$this->load->view('admin/footer');
			}
		}
		...
		public function krs_edit() {
			if (isset($_POST['submit'])) {
				$this->m_krs->ubah();
				$this->session->set_flashdata('simpan', 'KRS berhasil diperbaharui ...');
				redirect('admin/krs');
			} else {
				if ($krs = $this->uri->segment(3)) {
					$data = array(
					  'title' => 'Edit KRS Mahasiswa | Panel Admin SIINI',
					  'krs' => $this->db->get_where('tbl_krs', array('kd_krs' => $krs))->row_array()
					);
					$this->load->view('admin/header', $data);
					$this->load->view('admin/krs_edit');
					$this->load->view('admin/footer');
				} else {
					redirect('admin/krs');
				}
		...
		//hapus krs
		public function krs_hapus() {
			if ($kd_krs = $this->uri->segment(3)) {
				if(!empty($kd_krs)) {
